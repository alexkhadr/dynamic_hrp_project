...
from .standardize import (
    to_datetime_series, ffill_prune, weekly_last,
    pick_first_nonnull_column, to_numeric_bondaware
)
...
def load_raw_frames() -> dict[str, pd.DataFrame]:
    ...
    # RX1 sometimes has a header row as first data row
...
def split_ohlcv_blocks(df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    """
    For files like AT-03 where columns are:
      [..., '<TICKER> Comdty', '<TICKER> Open', '<TICKER> Low',
       '<TICKER> Last', '<TICKER> High', '<TICKER> Volume', ...].
    Identify each '<...> Comdty' anchor and slice the next 6 columns as a block.
    Returns {ticker: tidy_ohlcv_df} with columns: Date, Open, Low, Last, High, Volume, Ticker
    """
    tickers = [c for c in df.columns if "Comdty" in c]
    cleaned: dict[str, pd.DataFrame] = {}
    for t in tickers:
        base_idx = df.columns.get_loc(t)
        subcols = df.columns[base_idx: base_idx + 6]
...
def build_last_from_ohlcv(
    ...
    """Pivot tidy OHLCV (with 'Ticker' col) into wide panel of Last prices by ticker."""
    y = x.copy()
...
def build_fx_panel(Spot_FX: pd.DataFrame) -> pd.DataFrame:
    """
    Spot FX columns come as '<PAIR> - Date' and '<PAIR> - Last Price'.
    Build wide panel with columns EURUSD, USDJPY, etc.
    """
...
def build_ty1_series(TY1_Comdty: pd.DataFrame) -> pd.Series:
    """US 10Y note future (TY1) - convert bond-style quotes to decimal if needed."""
    df = TY1_Comdty.copy()
    ...
    ser = to_numeric_bondaware(df[col])
    ser.name = "TY1"
    return ser

def build_rx1_series(RX1_Comdty: pd.DataFrame) -> pd.Series:
    """Bund future (RX1/EU1); pick a sensible 'Last Price' column."""
    df = RX1_Comdty.copy()
    df["Date"] = to_datetime_series(df["Date"])
    df = df.dropna(subset=["Date"]).drop_duplicates(subset=["Date"]).set_index("Date")
    col = "Last Price  (R2)"
...
def build_equity_futs_panel(Equity_Futures: pd.DataFrame) -> pd.DataFrame:
    """Equity futures panel containing ES1, VG1, NK1 when available."""
...
def build_vol_index_panel(Volatility_Index: pd.DataFrame) -> pd.DataFrame:
    """Build VIX and V2X canonical series picking the first non-null among sensible candidates."""
...
def build_price_panels(
    ...
    """Assemble master daily panel, W-FRI weekly panel, and subpanels dictionary."""
